...
def font_paths() -> list[Path]:
    """Rubik font files bundled with the app (Regular + weight/style variants)."""
    names = (
        "Rubik-Regular.ttf",
        "Rubik-Medium.ttf",
        "Rubik-SemiBold.ttf",
...
def stylesheet_path() -> Path:
    if getattr(sys, "frozen", False):
        frozen_path = Path(sys._MEIPASS) / "ams2_ai" / "ui" / "styles" / "ams2.qss"
        if frozen_path.is_file():
            return frozen_path
    return Path(__file__).resolve().parent.parent / "ui" / "styles" / "ams2.qss"
